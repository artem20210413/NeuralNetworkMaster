...
//namespace NeuralNetwork_2._0_
{
    ...
    //        int numInput = 5; // number features
    //        int numHidden = 5;
    ...
    //      int cols, double v) // helper for ctor, Train // помощник для актера, Обучения
    //    {
    ...
    //        double[][] hoGrads = MakeMatrix(numHidden, numOutput, 0.0); // hidden-to-output weight gradients // градиенты веса idden-to-output
    //        double[] obGrads = new double[numOutput];                   // output bias gradients // градиенты смещения выходного сигнала

    //        double[][] ihGrads = MakeMatrix(numInput, numHidden, 0.0);  // input-to-hidden weight gradients // входные и скрытые градиенты веса
    //        double[] hbGrads = new double[numHidden];                   // hidden bias gradients

    //        double[] oSignals = new double[numOutput];                  // local gradient output signals - gradients w/o associated input terms // выходные сигналы локального градиента - градиенты без связанных входных условий
    //        double[] hSignals = new double[numHidden];                  // local gradient hidden node signals // сигналы скрытых узлов локального градиента

    //        // back-prop momentum specific arrays // массивы, специфичные для импульса задней опоры
    ...
    //        // правильный процент, используя принцип «победитель получает все»
    //        int numCorrect = 0;
    //        int numWrong = 0;
    //        double[] xValues = new double[numInput]; // inputs
    //        double[] tValues = new double[numOutput]; // targets цель
